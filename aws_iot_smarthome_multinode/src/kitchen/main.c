...
{
	if (state) {
		IOT_INFO("Turn on KitchenLights");
		led_write(KITCHEN_LEDMASK, KITCHEN_LEDMASK);
	} else {
		IOT_INFO("Turn off KitchenLights");
...
void KitchenLightsActuate_Callback(const char *pJsonString, uint32_t JsonStringDataLen, jsonStruct_t *pContext) {
	if (pContext != NULL) {
		...
		IOT_INFO("Delta - KitchenLights state changed to %d", temp);
...
{
	IOT_INFO("++++Smarthome Kitchen Node Startup++++");
