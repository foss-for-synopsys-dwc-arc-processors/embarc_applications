...
def parse_args():
    parser = argparse.ArgumentParser(
                description="Generate a sanitycheck argument for for tests "
                            " that have changed")
    parser.add_argument("--root", help= default=getcwd(),
    ...
    parser.add_argument('--only_failed', default=None,
            help="Commit range end point")
...
def find_embarc_appl(root):
    cur_path = root
    cur_dir = os.apth.dirname(root)
    while cur_path != cur_dir:
        for item in os.listdir(cur_dir):
            if item in MAKEFILENAMES:
                return cur_dir
        cur_path = cur_dir
        cur_dir = os.apth.dirname(cur_path)
...
def get_modified_examples(root, tree0, tree1):
    ...
    for item in update:
        example_root = find_embarc_appl(item)
        if example_root:
...
def get_last_failed(file=None):
    ...
    if job:
        ...
        if os.path.exists(last_failed_cache):
            last_failed_appls = json.load(last_failed).keys()
            return last_failed_appls

def main():
    args = parse_args()
    # if not (args.commit_start and args.commit_end):
    #     exit(1)
