...
struct DATA {
    ...
    char minute[2];
    char second[2];
    unsigned short people_num;
    unsigned short people_num_pre;          //historical data
    unsigned short people_num_pre_pre;      //Earlier historical data
    unsigned short people_num_buf;
    unsigned short people_num_pre_buf;
...
// Initialize queues as data buffers
    ...
    while (1) {
        ...
        memset(http_server_buf, 0, sizeof(http_server_buf));                //Initialize array
        memset(http_server_buf_1, 0, sizeof(http_server_buf_1));
        at_read(esp8266->p_at , http_server_buf , 1000);                    //Receive WIFI data
...
//The number of people below the threshold will not be reported to the cloud
            ...
            {
                EMBARC_PRINTF("There are %d people in the area. ", total_people_num_vaild);
...
int data_process(char *buff)
{
    if (strstr(buff, http_get) != NULL) {
        ...
        if (strstr(time_month, "01") != NULL)
            {strcpy(Month_trans, "Jan");} else if (strstr(time_month, "02") != NULL)
            {strcpy(Month_trans, "Feb");} else if (strstr(time_month, "03") != NULL)
            {strcpy(Month_trans, "Mar");} else if (strstr(time_month, "04") != NULL)
            {strcpy(Month_trans, "Apr");} else if (strstr(time_month, "05") != NULL)
            {strcpy(Month_trans, "May");} else if (strstr(time_month, "06") != NULL)
            {strcpy(Month_trans, "Jun");} else if (strstr(time_month, "07") != NULL)
            {strcpy(Month_trans, "Jul"); } else if (strstr(time_month, "08") != NULL)
            {strcpy(Month_trans, "Aug");} else if (strstr(time_month, "09") != NULL)
            {strcpy(Month_trans, "Sept");} else if (strstr(time_month, "10") != NULL)
            {strcpy(Month_trans, "Oct");} else if (strstr(time_month, "11") != NULL)
            {strcpy(Month_trans, "Nov");} else if (strstr(time_month, "12") != NULL)
        {strcpy(Month_trans, "Dec");}
        ...
        case 'A': {
            ...
            EMBARC_PRINTF("Number of people:  The number of people in area B is:%04d.\r\n", device_B.people_num);
            EMBARC_PRINTF("Number of people:  The number of people in area C is:%04d.\r\n", device_C.people_num);
            ...
            (device_A.people_num > device_A.people_num_pre_buf) ? (flag_buff[0] = 'I') : (flag_buff[0] = 'D');
            ...
            if (device_A.people_num > device_A.people_num_pre && device_A.people_num_pre > device_A.people_num_pre_pre)
            ...
            total_people_num = device_A.people_num + device_B.people_num + device_C.people_num;
        ...
        case 'B': {
            ...
            (device_B.people_num > device_B.people_num_pre_buf) ? (flag_buff[1] = 'I') : (flag_buff[1] = 'D');
            ...
            if (device_B.people_num > device_B.people_num_pre && device_B.people_num_pre > device_B.people_num_pre_pre)
            {   EMBARC_PRINTF("\n======================== Crowd status estimation ==========================\n\n");
        ...
        case 'C': {
            ...
            (device_C.people_num > device_C.people_num_pre_buf) ? (flag_buff[2] = 'I') : (flag_buff[2] = 'D');
            ...
            if (device_C.people_num > device_C.people_num_pre && device_C.people_num_pre > device_C.people_num_pre_pre)
            {   EMBARC_PRINTF("\n========================= Crowd status estimation ===========================\n\n");
...
int abnormal_data_processing(float var, float ave) {
    ...
    (ave - 5 * var < total_people_num < ave + 5 * var) ? (vaild_buff[0] = 'T') : (vaild_buff[0] = 'F');
...
int data_storage(char *buff) {
    ...
    fprintf(fp, "%s\r\n", buff);
    fprintf(fp, "A:%04d,B:%04d,C:%04d,total:%04d\r\n", device_A.people_num, device_B.people_num, device_C.people_num, total_people_num);
    fclose(fp);
    EMBARC_PRINTF("\n============================= Stores the data =================================\n\n");
...
void print_process(char *buff)
{
    if (memcmp(buff, "III", 3) == 0) // The number of peoplein A,B and C increase.
    {
        EMBARC_PRINTF("\n=============================== Flow of crowd =================================\n\n");
        EMBARC_PRINTF("Flow of crowd: The crowd is gathering. \n");
        state_III();
    }
    if (memcmp(buff, "DDD", 3) == 0) // The number of peoplein A,B and C decrease.
    {   EMBARC_PRINTF("\n=============================== Flow of crowd =================================\n\n");
    ...
    {   EMBARC_PRINTF("\n=============================== Flow of crowd =================================\n\n");
        EMBARC_PRINTF("Flow of crowd: Crowds move from areaA and C to area B \n");
        state_DID();
    }
...
{
    int length = 38;
    char *send_to_server_mid = (char *) malloc(strlen(first_word) + strlen(mid_word));
    strcpy(send_to_server_mid, first_word);
    strcat(send_to_server_mid, mid_word);

    char *send_to_server_all = (char *) malloc(strlen(send_to_server_mid) + strlen(last_word));
...
void variance(struct Queue * q, float * var, float ave)
{
    ...
    while ((temp != q->rear) && !isEmpty(q))
    {
        *var = (float)(*(q->pBase + temp) - ave) * (*(q->pBase + temp) - ave) / (q->len - 2);
