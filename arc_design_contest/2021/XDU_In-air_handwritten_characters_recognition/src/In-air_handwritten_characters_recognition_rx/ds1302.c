...
{
	time_extr[0]  = (time_rcv[0] >> 4);       //Thousands of Years
	time_extr[1]  = (time_rcv[0] &  0x0F);    //Hundreds of Years
	time_extr[2]  = (time_rcv[1] >> 4);       //Year ten
	time_extr[3]  = (time_rcv[1] &  0x0F);    //Year, ones

	time_extr[4]  = (time_rcv[2] >> 4);       //Month ten
	time_extr[5]  = (time_rcv[2] &  0x0F);    //Month, ones

	time_extr[6]  = (time_rcv[3] >> 4);       //day ten
	time_extr[7]  = (time_rcv[3] &  0x0F);    //day, ones

	time_extr[8]  = (time_rcv[4] >> 4);       //Hours, ten
	time_extr[9]  = (time_rcv[4] &  0x0F);    //Hours, ones

	time_extr[10] = (time_rcv[5] >> 4);       //Minutes, ten
	time_extr[11] = (time_rcv[5] &  0x0F);    //Minutes, ones

	time_extr[12] = (time_rcv[6] >> 4);       //Seconds, ten
	time_extr[13] = (time_rcv[6] &  0x0F);    //Seconds, ones
...
//Justify the parameter if needed
        // rtc_write_byte(DS1302_CONTROL_ADD,   0x00);			// turn off write protection
        // rtc_write_byte(DS1302_SEC_ADD,       0x80);			// pause the clock
        // rtc_write_byte(DS1302_CHARGER_ADD,   0xa9);         // trickle charge
        // rtc_write_byte(DS1302_YEAR_ADD,      0x21);	// year (20xx)
        // rtc_write_byte(DS1302_MONTH_ADD,     0x06);	// month
        // rtc_write_byte(DS1302_DATE_ADD,      0x07);	// day
        // rtc_write_byte(DS1302_HR_ADD,        0x16);	// hour
        // rtc_write_byte(DS1302_MIN_ADD,       0x23);	// minute
        // rtc_write_byte(DS1302_SEC_ADD,       0x00);	// second
        // rtc_write_byte(DS1302_DAY_ADD,       time_init[7]);	// day of week
        // rtc_write_byte(DS1302_SEC_ADD,       0x00);			// pause the clock
        // rtc_write_byte(DS1302_CONTROL_ADD,   0x80);			// turn on write protection
