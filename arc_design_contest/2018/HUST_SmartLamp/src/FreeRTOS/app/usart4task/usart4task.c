...
void suart_send_var_init(suart_send_struct* sd_ctr)
{
	sd_ctr->bit_flag = 0;
	sd_ctr->byte_now = 0;
	sd_ctr->send_flag = 0;
...
{
	uart4->gpio_control(GPIO_CMD_DIS_BIT_INT, (void *)GPIO_UART4IN_MASK);
	uart4_ctr.count=0;
	uart4_ctr.byte = 0;
	uart4_ctr.flag = 1;
 ...
 * */
		...
		#else//移到定时器中完成
		while(!(uart4_ctr.flag));//测试在定时器中开关外部中断，测试效果

		write_fifo(&usart4_fifo,&uart4_ctr.byte,1);
		uart4_ctr.timeout = 0x80;
		uart4_ctr.bytenum++;//可能需要判断数据量过大，进行断帧
		if(uart4_ctr.bytenum>USART4_FIFO_LEN)
		{
				xQueueSendFromISR(USART4_Queue,&uart4_ctr.bytenum,&xHigherPriorityTaskWoken);//向队列中发送数据
				uart4_ctr.bytenum = 0;
...
void u4app_send(uint8_t*str,uint8_t len)//内部调用函数
{
	...
	#if 1//发送到发送fifo中
	write_fifo(&usart4_send_fifo,str,len);
//	TIM_Cmd(TIM5,ENABLE);
...
void usart4_init(void)
{
    ...
    init_fifo(&usart4_fifo,USART4_FIFO_LEN,usart4_fifo_data);//初始化串口4 的接收fifo
	init_fifo(&usart4_send_fifo,USART4_SEND_LEN,usart4_send_data);//初始化串口4 的接收fifo
    suart_var_init(&uart4_ctr);
    suart_send_var_init(&send_ctr4);
...
void USART4_app_task(void *pvParameters)
{
	...
	while(1)
	{
		if(xQueueReceive(USART4_Queue,&nlen,0))//接收数据
		{
			...
			vPortFree(buffer);
//			dprintf("u4fifo:");
//			for(nlen=0;nlen<USART4_FIFO_LEN;nlen++)
//			{
//				dprintf("%2x",usart4_fifo_data[nlen]);
//			}
//			dprintf("\r\n");
			#endif
			//处理触控板按键数据
...
|return|enable|power | wifi | sp 1 | boot |wifiset| nonsense
														|	sp 0 |0-7  =  light1-8
...
|			 |power | boot |wifiset| wifi |0-8 = light 0-8
*/
...
//		sendpack2server(0x0002);
	    sprintf(say2server_temp,"light mode:touch off");
	    say2server(say_did, say2server_temp);
}
...
void touch_process_poweron(void)//poweron的处理 进入自动调光
{
	...
	//发送包0002
//	sendpack2server(0x0002);
    sprintf(say2server_temp,"light mode:touch on");
    say2server(say_did, say2server_temp);
	//控制wifi灯
	if(touch_flag&(0x0001<<14))
	{
//		dprintf("try send wifi on\r\n");
...
//						                    		mp3_play(1);
		...
		//发送0002
//		sendpack2server(0x0002);
        sprintf(say2server_temp,"light mode:touch %1d",touch_flag&0x000f);
        say2server(say_did, say2server_temp);
			//控制wifi灯
		if(touch_flag&(0x0001<<14))
		{
