...
void mcu2wifi_product_info(void)
{
	...
	for(uint8_t i=2;i<0x73-1;i++) checksum = checksum + send_data[i];
...
void mcu2wifi_set_cfg_mode(uint8_t mode)
{
	...
	send_data[8] = mode;
	//计算校验和（包长度至校验和前一byte所有数据之和对256取余）
	for(uint8_t i=2;i<0x0a-1;i++) checksum = checksum + send_data[i];
...
void mcu2wifi_wifi_statu(uint8_t *receive_data)
{
	...
	if((statu)   %2 == 1) EMBARC_PRINTF("SoftAP mode is open\r\n");	
	...
	if((statu>>5)%2 == 1) EMBARC_PRINTF("The WiFi module is connected to the M2M server\r\n");
}
