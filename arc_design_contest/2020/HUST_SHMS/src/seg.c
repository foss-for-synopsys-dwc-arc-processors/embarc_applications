...
static void cifar10_preprocessing(const void * image_, mli_tensor * net_input_) {
	...
	d_type * const dst = (d_type * const)net_input_->data; //y_mem_buf YCCM 12*32*32

	// Copying data  to input tensor with subtraction of average.
	// Data shift may be required depending on tensor format 相当于将输入像素归一化到[-1,1)
...
int process(uint8_t* images) {
	...
	cifar10_preprocessing(images, cifar10_cf_net_input);
...
// Check result
	if (MLI_STATUS_OK == mli_hlp_fx_tensor_to_float(cifar10_cf_net_output, pred_data, output_elements)) {
		...
		if(*(pred_data+1)>*pred_data)
		{
			printf("somebody falls\n\n\n");
		}
		else
		{
		}
